...
function getFfmpegPath() {
  ...
  if (platform === "darwin") {
    return isDev
      ? "ffmpeg-mac/ffmpeg"
      : join(window.process.resourcesPath, "ffmpeg");
  }
...
function getFfprobePath() {
  ...
  if (platform === "darwin") {
    return isDev
      ? "ffmpeg-mac/ffprobe"
      : join(window.process.resourcesPath, "ffprobe");
  }
  ...
  if (!subPath)
    throw new Error(`${i18n.t("Unsupported platform")} ${platform}`);
...
export function findNearestKeyFrameTime({ frames, time, direction, fps }) {
  ...
  const nearestFrame = sortBy(keyframes, (keyframe) =>
...
async function cut({
  ...
  console.log(
    "Exporting from",
    cuttingStart ? cutFrom : "start",
    "to",
    cuttingEnd ? cutTo : "end"
  );
  ...
  const avoidNegativeTsArgs = cuttingStart
    ? ["-avoid_negative_ts", "make_zero"]
    : [];

  const inputArgs = flatMap(copyFileStreamsFiltered, ({ path }) => [
    "-i",
    path,
  ]);
  const inputCutArgs = ssBeforeInput
    ? [...cutFromArgs, ...inputArgs, ...cutToArgs, ...avoidNegativeTsArgs]
    : [...inputArgs, ...cutFromArgs, ...cutToArgs];

  const rotationArgs =
    rotation !== undefined
      ? ["-metadata:s:v:0", `rotate=${360 - rotation}`]
      : [];
  ...
  const ffmpegArgs = [
    ...
    ...flatMapDeep(copyFileStreamsFiltered, ({ streamIds }, fileIndex) =>
      streamIds.map((streamId) => ["-map", `${fileIndex}:${streamId}`])
    ),
    "-map_metadata",
    "0",
...
function getOutFileExtension({ isCustomFormatSelected, outFormat, filePath }) {
  return isCustomFormatSelected
    ? `.${getExtensionForFormat(outFormat)}`
    : extname(filePath);
}
...
export async function cutMultiple({
  ...
  for (const { start, end, name } of segments) {
    const cutFromStr = formatDuration({
      seconds: start,
      fileNameFriendly: true,
    });
    const cutToStr = formatDuration({ seconds: end, fileNameFriendly: true });
    const segNamePart = name ? `-${filenamify(name)}` : "";
    const cutSpecification = `${cutFromStr}-${cutToStr}${segNamePart}`.substr(
      0,
      200
    );
...
export async function mergeFiles({
  ...
  const concatTxt = paths
    .map((file) => `file '${join(file).replace(/'/g, "'\\''")}'`)
    .join("\n");
...
export async function autoMergeSegments({
  customOutDir,
  sourceFile,
  isCustomFormatSelected,
  outFormat,
...
export async function renderWaveformPng({
  ...
  const args2 = [
    ...
    "-",
    "-filter_complex",
    `aformat=channel_layouts=mono,showwavespic=s=640x120:scale=sqrt:colors=${color}`,
    "-frames:v",
    "1",
    "-vcodec",
    "png",
...
export function getOneRawFrame({
  path,
  inWidth,
  inHeight,
  seekTo,
...
export function encodeLiveRawStream({
  path,
  inWidth,
  inHeight,
  seekTo,
...
export async function fixInvalidDuration({
  filePath,
  fileFormat,
  customOutDir,
}) {
