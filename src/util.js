...
export function getOutPath(customOutDir, filePath, nameSuffix) {
  ...
  return path.join(
    getOutDir(customOutDir, filePath),
...
export function setFileNameTitle(filePath) {
  ...
  document.title = filePath
    ? `${appName} - ${path.basename(filePath)}`
    : appName;
}

export function setFileNameTitle1(filePath) {
  const appName = "LosslessCut1";
  document.title = filePath
    ? `${appName} - ${path.basename(filePath)}`
    : appName;
}
